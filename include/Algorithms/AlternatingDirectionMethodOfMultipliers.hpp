 ...
 */
template<class Scalar>
void LeastAbsoluteDeviationMethod(const MatrixBase<Scalar>& A,
		const VectorBase<Scalar>& b, const Scalar rho, VectorBase<Scalar>& u,
		VectorBase<Scalar>& x, VectorBase<Scalar>& z, int& number, Scalar& e) {
	int count = number;
	int m = 0;
	int n = 0;
	number = 0;
