...
private:
	...
	enum StepType {
		Normal, Optimal, Unbounded
	};

	/** type of the algorithm */
	enum AlgoType {
		Starting, Success, Unbound, NotFeasible
	};
...
public:
	...
	SimplexSolver(const Matrix& A, const Vector& b, const Vector& c);
	//%}
...
public:
	 ...
	 */
	static inline void solveBlockingSystem(const Matrix& A, const Vector& b,
			const std::vector<index>& indb, Vector& x);
	 ...
	 *		/param x:			the result on output
	 */
	static inline void solveCompleteBlockingSystem(const Matrix& A,
			const Vector& b, const std::vector<index>& indb, Vector& x);
	 ...
	 */
	static inline StepType oneSimplexStep(const Matrix& A, const Vector& b,
			const Vector& c, std::vector<index>& indb,
			std::vector<index>& indn);

	static inline bool findPivoting(const Vector& xb, const Vector& d,
			const int q, std::vector<index>& indb, std::vector<index>& indn);
	 ...
	 *		/param indz:
	 */
	static inline bool findFeasiblePoint(const index m, const index n,
			const Matrix& A, const Vector& b, std::vector<index>& indb,
			std::vector<index>& indn, std::vector<index>& indz);
	 ...
	 */
	static inline AlgoType simplexMethodWithInitalization(const int m,
			const int n, const Matrix& A, const Vector& b, const Vector& c,
			Vector& x, scalar& val, std::vector<index>& indb,
			std::vector<index>& indn);
	 ...
	 *		/param indz:		used Z
	 */
	static inline AlgoType simplexMethodWithIndices(const int m, const int n,
			const Matrix& A, const Vector& b, const Vector& c, Vector& x,
			scalar& val, std::vector<index>& indb, std::vector<index>& indn,
...
SimplexSolver<kernel>::SimplexSolver() :
		__m(0), __n(0), __type(Starting) {
}

template<class kernel>
SimplexSolver<kernel>::SimplexSolver(const Matrix& A, const Vector& b,
		const Vector& c) :
		__n(A.cols()), __m(A.rows()), __A(A), __b(b), __c(c), __type(Starting) {
...
typename SimplexSolver<kernel>::AlgoType SimplexSolver<kernel>::solve() {
	...
	if (findFeasiblePoint(__m, __n, __A, __b, indb, indn, indz)) {
		...
		__type = simplexMethodWithIndices(__m, __n, __A, __b, __c, __x, __val,
				indb, indn, indz);
		return __type;
	} else {
		__type = NotFeasible;
...
template<class kernel>
void SimplexSolver<kernel>::solveBlockingSystem(const Matrix& A,
		const Vector& b, const std::vector<index>& indb, Vector& x) {
	Matrix B;
...
template<class kernel>
void SimplexSolver<kernel>::solveCompleteBlockingSystem(const Matrix& A,
		const Vector& b, const std::vector<index>& indb, Vector& x) {
	x.resize(A.cols());
	Vector xb;
...
template<class kernel>
bool SimplexSolver<kernel>::findPivoting(const Vector& xb, const Vector& d,
		const int q, std::vector<index>& indb, std::vector<index>& indn) {
	int p = -1;
	scalar min = INFTY;
...
typename SimplexSolver<kernel>::StepType SimplexSolver<kernel>::oneSimplexStep(
		...
		std::vector<index>& indb, std::vector<index>& indn) {
	Matrix B, N;
	B.columnBlockFromMatrix(A, indb);
	Vector xb = B.solve(b);
...
template<class kernel>
bool SimplexSolver<kernel>::findFeasiblePoint(const index m, const index n,
		const Matrix& A, const Vector& b, std::vector<index>& indb,
		std::vector<index>& indn, std::vector<index>& indz) {
	...
	Vector x;
	scalar val;
	AlgoType t = simplexMethodWithInitalization(m, n, AE, b, e, x, val, iindb,
			iindn);

	// compute indb and indn. index less than n is kept
	indb.clear();
	indb.reserve(iindb.size());
	indn.clear();
	indn.reserve(iindn.size());
	indz.clear();
	indz.reserve(iindb.size());
...
typename SimplexSolver<kernel>::AlgoType SimplexSolver<kernel>::simplexMethodWithInitalization(
		const int m, const int n, const Matrix& A, const Vector& b,
		const Vector& c, Vector& x, scalar& val, std::vector<index>&indb,
		std::vector<index>&indn) {
	StepType type;
	do {
...
typename SimplexSolver<kernel>::AlgoType SimplexSolver<kernel>::simplexMethodWithIndices(
	...
	for (int i = 0; i < zsize; ++i)
		iindb.push_back(n + i);
	AlgoType t = simplexMethodWithInitalization(m, n, nA, nb, nc, nx, val,
			iindb, indn);
	indb.clear();
	indb.reserve(iindb.size());
