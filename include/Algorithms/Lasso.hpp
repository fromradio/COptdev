...
class LassoProximalSolver: public GeneralSolver<typename Problem::KernelTrait,
		Time> {
private:
...
public:
	...
	LassoProximalSolver(const Problem& s, const scalar mu = 0.5,
			const scalar beta = 1.0, const index maxiteration = 10000);
 ...
 */
template<class Problem, class Time = NoTimeStatistics>
class LassoADMMSolver: public GeneralSolver<typename Problem::KernelTrait, Time> {
private:
	typedef typename Problem::KernelTrait::index index;
	typedef typename Problem::KernelTrait::scalar scalar;
	typedef typename Problem::KernelTrait::Vector Vector;
...
public:
	...
	//%{
	LassoADMMSolver(const Problem& p, const scalar rho = 0.5,
			const index maxiteration = 1000, const scalar thresh = 1e-8);
	//%}
 ...
 */
template<class Problem, class Time = NoTimeStatistics>
class FISTALassoSolver: public GeneralSolver<typename Problem::KernelTrait, Time> {
private:
...
public:
	...
	/** constructor and deconstructor */
	//%{
	FISTALassoSolver(const Problem& p, const scalar L = 1.0,
			const index maxiteration = 100000, const scalar thresh = 1e-6,
			const scalar beta = 2.0, bool optimalused = false);
	//%}
	...
	/** find the next L */
	inline scalar findConstant(const Vector& ply, const Vector& y,
			const scalar L, const scalar beta);
...
class LassoProblem: public VectorProblem<kernel> {
private:
	typedef typename kernel::scalar scalar;
	typedef typename kernel::Matrix Matrix;
	typedef typename kernel::Vector Vector;
...
public:
	...
	LassoProblem(const Matrix& A, const Vector& b, const scalar lambda = 0.5);
...
template<class Problem, class Time>
LassoProximalSolver<Problem, Time>::LassoProximalSolver(const Problem& s,
		const scalar mu, const scalar beta, const index maxiteration) :
		__p(s), __mu(mu), __beta(beta), __maxiteration(maxiteration) {
	this->doCompute();
}

template<class Problem, class Time>
typename LassoProximalSolver<Problem, Time>::Vector LassoProximalSolver<Problem,
		Time>::fGradient(const Vector& x) {
	return 2 * (__mtm * x - __mt * __p.obB());
}

template<class Problem, class Time>
typename LassoProximalSolver<Problem, Time>::Vector LassoProximalSolver<Problem,
		Time>::gProximal(const Vector& x) {
...
template<class Problem, class Time>
typename LassoProximalSolver<Problem, Time>::scalar LassoProximalSolver<Problem,
		Time>::f(const Vector& x) {
	return (__p.matA() * x - __p.obB()).squaredNorm();
}

template<class Problem, class Time>
typename LassoProximalSolver<Problem, Time>::scalar LassoProximalSolver<Problem,
		Time>::fMu(const Vector& x, const Vector& y) {
...
template<class Problem, class Time>
const typename LassoProximalSolver<Problem, Time>::Vector& LassoProximalSolver<
...
template<class Problem, class Time>
LassoADMMSolver<Problem, Time>::LassoADMMSolver(const Problem& p,
		const scalar rho, const index maxiteration, const scalar thresh) :
		GeneralSolver<typename Problem::KernelTrait, Time>(maxiteration,
				thresh), __p(p), __rho(rho) {
	this->compute();
}
...
template<class Problem, class Time>
void LassoADMMSolver<Problem, Time>::xSubProblem(const Vector& zk,
		const Vector& yk, Vector&xn) {
	Vector rhd = __mt * __p.obB() + 1.0 / (2.0 * __rho) * (zk - yk);
	xn = __linear_solver.solve(rhd);
}

template<class Problem, class Time>
void LassoADMMSolver<Problem, Time>::zSubProblem(const Vector& xn,
		const Vector& yk, Vector& zn) {
	for (int i = 0; i < zn.size(); ++i)
		zn(i) = computeProximal(AbsFunction(), xn(i) + yk(i),
				__rho * __p.lambda());
}

template<class Problem, class Time>
void LassoADMMSolver<Problem, Time>::ySubProblem(const Vector& xn,
		const Vector& zn, Vector& yn) {
	yn = yn + (xn - zn);
}
...
template<class Problem, class Time>
const typename LassoADMMSolver<Problem, Time>::Vector& LassoADMMSolver<Problem,
		Time>::result() const {
	if (!__is_solved)
		std::cerr << "Warning: the problem is not successfully solved yet!"
				<< std::endl;
	return __x;
}
...
template<class Problem, class Time>
FISTALassoSolver<Problem, Time>::FISTALassoSolver(const Problem& p,
		const scalar L, const index maxiteration, const scalar thresh,
		const scalar beta, bool optimalused) :
		GeneralSolver<kernel, Time>(maxiteration, thresh), __p(p), __L(L), __beta(
				beta), __optimal_constant(optimalused) {
	this->compute();
}

template<class Problem, class Time>
typename FISTALassoSolver<Problem, Time>::scalar FISTALassoSolver<Problem, Time>::fFunction(
		const Vector& x) {
	return (__p.matA() * x - __p.obB()).squaredNorm();
}

template<class Problem, class Time>
typename FISTALassoSolver<Problem, Time>::Vector FISTALassoSolver<Problem, Time>::fGradient(
		const Vector & x) {
	return 2 * (__p.matA().transMulti(__p.matA() * x) - __atb);
}

template<class Problem, class Time>
typename FISTALassoSolver<Problem, Time>::scalar FISTALassoSolver<Problem, Time>::qFunction(
		const Vector& x, const Vector& y, const scalar L) {
...
template<class Problem, class Time>
void FISTALassoSolver<Problem, Time>::pArgmin(const Vector& y, const scalar L,
		Vector& x) {
	x.resize(y.size());
...
template<class Problem, class Time>
typename FISTALassoSolver<Problem, Time>::scalar FISTALassoSolver<Problem, Time>::findConstant(
		const Vector& ply, const Vector& y, const scalar L, const scalar beta) {
	if (beta < 1.0)
		throw COException(
				"please make sure the ratio factor beta of FISTA solver is bigger than 1.0!");
	scalar nL = L;
...
template<class Problem, class Time>
const typename FISTALassoSolver<Problem, Time>::Vector& FISTALassoSolver<
		Problem, Time>::result() const {
	return __x;
}
...
template<class kernel>
LassoProblem<kernel>::LassoProblem(const Matrix& A, const Vector& b,
		const scalar lambda) :
		__A(A), __b(b), __lambda(lambda) {
}
