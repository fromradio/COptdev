...
class APGSolver: public GeneralSolver<kernel, Time, typename kernel::Matrix> {
private:
...
public:
	APGSolver(const Matrix& D, const scalar lam, const index maxiteration =
			10000);
...
template<class kernel, class Time>
APGSolver<kernel, Time>::APGSolver(const Matrix& D, const scalar lam,
		const index maxiteration) :
		__D(D), __lam(lam), __maxiteration(maxiteration) {
	this->doCompute();
}
...
void APGSolver<kernel, Time>::softThreshold(T& t, const scalar mu) {
	std::for_each(t.begin(), t.end(),
			[&mu](scalar& s) {s=(s>mu)?(s-mu):((s<-mu)?(s+mu):0);});
}
...
template<class InputType, class OutputType = InputType,
		class ConjugateType = InputType>
class COPTMap: public COPTObject {
public:
	virtual ~COPTMap() {
	}
...
class CompletionMap: public COPTMap<Matrix, typename Matrix::DVector> {
private:
...
public:
	...
	CompletionMap(const index nr, const index nc, const index nnz,
...
template<class Matrix>
CompletionMap<Matrix>::CompletionMap(const index nr, const index nc,
		const index nnz, const index *ir, const index *jc) :
		__nr(nr), __nc(nc), __nnz(nnz), __ir(nullptr), __jc(nullptr) {
	__ir = new index[nnz];
...
template<class Matrix>
typename CompletionMap<Matrix>::DVector CompletionMap<Matrix>::operator()(
		const Matrix& mat) {
...
class APGLSolver: public GeneralSolver<kernel, NoTimeStatistics,
		typename kernel::Matrix> {
private:
...
public:
	...
	APGLSolver(const index nr, const index nc, const index nnz, const index *ir,
			const index *jc, const Vector& b, const scalar beta = 0.5,
			const scalar mu = 0.005);
	APGLSolver(const SpMatrix& spmat, const scalar beta = 2.0, const scalar mu =
			0.000005);
...
template<class kernel>
APGLSolver<kernel>::APGLSolver(const index nr, const index nc, const index nnz,
		const index *ir, const index *jc, const Vector& b, const scalar beta,
		const scalar mu) :

		__m(nr), __n(nc), __cm(nr, nc, nnz, ir, jc), __b(b), __beta(beta), __mu(
				mu) {
...
template<class kernel>
APGLSolver<kernel>::APGLSolver(const SpMatrix& spmat, const scalar beta,
		const scalar mu) :
		__m(spmat.rows()), __n(spmat.cols()), __cm(spmat.rows(), spmat.cols(),
				spmat.elementSize(), spmat.rowIndex(), spmat.columnPointer()), __beta(
				beta), __mu(mu) {
 ...
 */
template<class Matrix>
class LowRankALM: public GeneralSolver<typename Matrix::Kernel,
		NoTimeStatistics, typename Matrix::DMatrix> {
public:
...
template<class Matrix>
LowRankALM<Matrix>::LowRankALM(const Matrix& d, const scalar lambda,
		const scalar mu) :
		__d(d), __lambda(lambda), __mu(mu), __s(d) {
	this->compute();
}
