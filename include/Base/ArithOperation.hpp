...
void checkDimension(const Vector &v1, const Vector &v2) {
	if (v1.size() != v2.size())
		throw COException(
				"COPT error: The size of two vectors are not consistent!");
}
...
template<class Vector>
typename Vector::podscalar norm(const Vector& vec, int l,
		const vector_object&) {
...
template<class Vector>
typename Vector::podscalar norm(const Vector &vec, typename Vector::podscalar l,
		const vector_object&) {
...
typename VectorIterator::value_type mean(VectorIterator begin,
		VectorIterator end) {
	typename VectorIterator::value_type vec((*begin).dimension());
	int n = 0;
	std::for_each(begin, end,
			[&vec,&n](typename VectorIterator::value_type& v) {vec=vec+v;++n;});
	vec.scale(1.0 / n);
	return vec;
}
