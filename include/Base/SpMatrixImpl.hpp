...
TripletBase<scalar, index>::TripletBase(const index r, const index c,
		const scalar v) :
		__r(r), __c(c), __v(v) {
}
...
template<class Triplet>
bool columnComparison<Triplet>::operator()(const Triplet& t1,
		const Triplet& t2) {
...
void SpMatrixBase<scalar, index>::judgeRationality() {
	for (index i = 0; i < __elesize; ++i) {
		if (__rowind[i] >= __rows)
			throw COException(
					"Sparse matrix not rational: row index out of range!");
	}
}

template<class scalar, class index>
const typename SpMatrixBase<scalar, index>::scalar SpMatrixBase<scalar, index>::__zero =
		static_cast<scalar>(0.0);

template<class scalar, class index>
SpMatrixBase<scalar, index>::SpMatrixBase() :
		__rows(0), __cols(0), __elesize(0), __colptr(NULL), __rowind(NULL), __vals(
				NULL) {
}

template<class scalar, class index>
SpMatrixBase<scalar, index>::SpMatrixBase(const index rows, const index cols,
		const index elesize, const index* colptr, const index* rowind,
		const scalar* vals) :
		__rows(rows), __cols(cols), __elesize(elesize), __colptr(
				new index[cols + 1]), __rowind(new index[elesize]), __vals(
				new scalar[elesize]) {
...
template<class scalar, class index>
SpMatrixBase<scalar, index>::SpMatrixBase(const SpMatrixBase& mat) :
		__colptr(NULL), __rowind(NULL), __vals(NULL) {
	setSparseMatrix(mat.rows(), mat.cols(), mat.elementSize(),
			mat.columnPointer(), mat.rowIndex(), mat.values());
}
...
template<class scalar, class index>
void SpMatrixBase<scalar, index>::setSparseMatrix(const index rows,
		const index cols, const index elesize, const index* colptr,
		const index* rowind, const scalar* vals) {
	clear();
	__rows = rows;
	__cols = cols;
	__elesize = elesize;
...
template<class scalar, class index>
SpMatrixBase<scalar, index>& SpMatrixBase<scalar, index>::operator=(
...
template<class scalar, class index>
void SpMatrixBase<scalar, index>::setFromTriplets(const index rows,
		const index cols, std::vector<Triplet>& triplets) {
	clear();
	__rows = rows;
	__cols = cols;
	...
	for (index i = 0; i < triplets.size(); ++i) {
		if (triplets[i].columnIndex() > colind) {
			...
			std::sort(triplets.begin() + ip, triplets.begin() + i,
					rowComparison<Triplet>());
			ip = i;
		}
	}
	// last sort
	...
	index i = 0;
	for (typename std::list<index>::iterator iter = rowinds.begin();
			iter != rowinds.end(); ++iter, ++i)
		__rowind[i] = *iter;

	__vals = new scalar[vals.size()];
	i = 0;
	for (typename std::list<scalar>::iterator iter = vals.begin();
			iter != vals.end(); ++iter, ++i)
		__vals[i] = *iter;
...
void SpMatrixBase<scalar, index>::setFromTriplets(const index rows,
		const index cols, const InputIterator& begin,
	...
	for (InputIterator iter = begin; iter != end; ++iter) {
		...
		if (iter == begin) {
			...
			++count;
				...
				&& iter->columnIndex() == previter->columnIndex()) {
...
template<class scalar, class index> template<class InputIterator>
void SpMatrixBase<scalar, index>::fastSetFromTriplets(const index rows,
		const index cols, const InputIterator& begin,
		const InputIterator& end) {
	clear();
	__rows = rows;
...
template<class scalar, class index>
const scalar& SpMatrixBase<scalar, index>::operator()(const index i,
		const index j) const {
...
SpMatrixBase<scalar, index> SpMatrixBase<scalar, index>::operator*(
	...
	for (index c = 0; c < __cols; ++c) {
		...
		for (index r = ci; r < cn; ++r) {
			...
			for (index mc = 0; mc < mat.cols(); ++mc) {
				index mci = mat.columnPointer()[mc], mcn =
						mat.columnPointer()[mc + 1];
				for (index mr = mci; mr < mcn; ++mr) {
					if (c == mat.rowIndex()[mr])
						tris.push_back(
								Triplet(rind, mc,
										__vals[r] * mat.values()[mr]));
				}
			}
		}
	}
	...
	for (typename std::list<Triplet>::iterator iter = tris.begin();
			iter != tris.end(); ++iter)
...
SpMatrixBase<scalar, index> SpMatrixBase<scalar, index>::operator+(
	...
	for (index c = 0; c < __cols; ++c) {
		colptr[c] = count;
		index ci1 = __colptr[c], cn1 = __colptr[c + 1], ci2 =
				mat.columnPointer()[c], cn2 = mat.columnPointer()[c + 1];
		index i1 = ci1, i2 = ci2;
		while (i1 < cn1 || i2 < cn2) {
			if (i1 < cn1 && i2 < cn2) {
	...
	index i = 0;
	for (typename std::list<index>::iterator iter = inds.begin();
			iter != inds.end(); ++iter, ++i)
		rowind[i] = *iter;
	i = 0;
...
template<class scalar, class index>
typename SpMatrixBase<scalar, index>::Vector SpMatrixBase<scalar, index>::operator*(
...
template<class scalar, class index>
VectorBase<scalar, index> SpMatrixBase<scalar, index>::solve(
		const VectorBase<scalar, index>& vec) {
